...
var ui = function(suite) {
  ...
  suite.on('pre-require', function(context) {
    ...
    */
    ...
    Object.keys(assertFns).forEach(function(name) {
      assert[name] = function() {
        assertionCount++;
        assertFns[name].apply(null, arguments);
      }
    ...
    */
    ...
    function addTest(title, expect, test) {
      suites[0].addTest(new Test(title, wrapTestFunction(test, function(test, done) {
        ...
        currentDoneFn = done;
        context.stop();
        QUnit.config.current = { assertions: [] };
        QUnit.config.current.testEnvironment = this;
        QUnit.current_testEnvironment = this;
